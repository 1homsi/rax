...
use raxon_core::{Arena, LayoutStyle};
use raxon_reactive::{create_effect, Effect};
...
impl Tree {
    ...
    pub fn set_content_size(&mut self, id: WidgetId, size: raxon_core::Size) {
        if self.nodes.get(id.0).is_some() {
    ...
    pub fn set_backdrop(&mut self, color: raxon_core::Color) {
    ...
    pub fn set_frame(&mut self, id: WidgetId, rect: raxon_core::Rect) {
        if self.nodes.get(id.0).is_some() {
