...
//!
//! ```
//! use raxon_view::{column, text, button, mount, View};
//! use raxon_dom::{Host, RecordingBackend, Tree};
//! use raxon_reactive::create_signal;
//!
//! fn counter(count: raxon_reactive::Signal<i32>) -> impl View {
//!     column((
...
thread_local! {
    static FPS_SIGNAL: std::cell::Cell<Option<raxon_reactive::Signal<f32>>> =
...
pub fn use_fps() -> raxon_reactive::Signal<f32> {
    let existing = FPS_SIGNAL.with(|s| s.get());
    if let Some(sig) = existing {
        return sig;
    }
    let sig = raxon_reactive::create_signal(60.0f32);
