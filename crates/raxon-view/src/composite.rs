...
use raxon_core::{AlignItems, Color, EdgeInsets, JustifyContent};
use raxon_dom::{GesturePhase, Tree, WidgetId};
...
///
/// ```
/// use raxon_view::modal;
/// use raxon_reactive::create_signal;
///
/// let open = create_signal(false);
/// let v = modal(open, move || open.set(false), || raxon_view::text("Hello"));
/// ```
...
/// # Example
/// ```rust
/// use raxon_view::{fade_transition, text};
/// use raxon_reactive::create_signal;
///
...
///
/// ```
/// use raxon_view::bottom_sheet;
/// use raxon_reactive::create_signal;
///
/// let open = create_signal(false);
/// let v = bottom_sheet(open, move || open.set(false), || raxon_view::text("Sheet body"));
/// ```
...
///
/// ```
/// use raxon_view::toast;
/// use raxon_reactive::create_signal;
///
/// let msg: raxon_reactive::Signal<Option<String>> = create_signal(None);
/// let v = toast(msg);
/// ```
...
/// # Example
/// ```rust
/// use raxon_view::{grid, text, boxed};
///
...
pub fn network_image(url: impl Into<String>, placeholder: impl Into<String>) -> impl View {
    ...
    // Kick off the fetch.
    let res = raxon_net::get(url);
    create_effect(move || {
        use raxon_async::ResourceState;
...
/// [`raxon_runtime::last_panic`] to surface panics as a visible overlay in
...
///
/// ```no_run
/// use raxon_view::{error_overlay, stack};
/// use raxon_reactive::create_signal;
///
/// // let msg = create_signal(raxon_runtime::last_panic());
/// // let view = stack((your_app_view, error_overlay(msg)));
/// ```
pub fn error_overlay(message: raxon_reactive::Signal<Option<String>>) -> impl View {
    dynamic(move || match message.get() {
        Some(msg) => boxed(
            column((
                boxed(
...
/// # Example
/// ```rust
/// use raxon_view::swipe_actions;
/// use raxon_core::Color;
/// use std::sync::Arc;
///
/// let view = swipe_actions(
///     || raxon_view::text("My item"),
///     vec![
...
///
/// ```rust
/// use raxon_view::{dev_tools, stack, text};
///
...
/// # Example
/// ```rust
/// use raxon_view::{section_list, Section, text, boxed, row, spacer};
/// use raxon_core::Color;
///
/// section_list(
///     vec![
...
/// # Example
/// ```rust
/// use raxon_view::{lazy_column, text, boxed};
///
...
/// # Example
/// ```rust
/// use raxon_view::{reactive_list, text, boxed};
/// use raxon_reactive::create_signal;
///
...
pub fn pan_animation(spring_back: bool) -> (Signal<f32>, Signal<f32>, impl FnMut(PanInfo)) {
    ...
    let handler = move |info: PanInfo| {
        ...
        if spring_back && info.phase == GesturePhase::Ended {
            ...
            let sx = x;
            let spring_x = raxon_anim::spring(sx.get(), 0.0, raxon_anim::Spring::default());
            create_effect(move || sx.set(spring_x.get()));
            let sy = y;
            let spring_y = raxon_anim::spring(sy.get(), 0.0, raxon_anim::Spring::default());
...
/// # Example
/// ```rust
/// use raxon_view::{wrap, chip};
/// use raxon_reactive::create_signal;
///
/// let selected = create_signal(0usize);
/// let tags: Vec<_> = (0..8)
///     .map(|i| raxon_view::boxed(chip(format!("Tag {i}"), selected.get() == i, move || selected.set(i))))
///     .collect();
/// let v = wrap(8.0, tags);
/// ```
...
/// # Example
/// ```rust
/// use raxon_view::{banner, BannerKind};
/// use raxon_reactive::create_signal;
///
/// let visible = create_signal(true);
...
/// # Example
/// ```rust
/// use raxon_view::{collapsible, text};
/// use raxon_reactive::create_signal;
///
/// let open = create_signal(false);
/// let v = collapsible(
...
/// # Example
/// ```rust
/// use raxon_view::{carousel, text, boxed};
/// use raxon_reactive::create_signal;
///
...
/// # Example
/// ```rust
/// use raxon_view::{keyboard_avoiding_view, text_input};
/// use raxon_reactive::create_signal;
///
...
/// # Example
/// ```rust
/// use raxon_view::{infinite_scroll, text, boxed};
/// use raxon_reactive::create_signal;
///
...
/// # Example
/// ```rust
/// use raxon_view::{status_bar_spacer, column, text};
///
/// let v = column((
///     raxon_view::boxed(status_bar_spacer()),
///     raxon_view::boxed(text("Content below status bar")),
/// ));
/// ```
...
/// # Example
/// ```rust
/// use raxon_view::{list_with_header, text, boxed};
/// use raxon_reactive::create_signal;
///
/// let items: raxon_reactive::Signal<Vec<String>> = create_signal(vec![]);
...
/// # Example
/// ```rust
/// use raxon_view::{sticky_header, text};
///
