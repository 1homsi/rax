...
//! # Example
//! ```no_run
//! use raxon_view::{register_component, ComponentProps};
//! use raxon_view::text;
//! use raxon_view::modifier::ViewExt;
//!
//! // Register a custom button factory at startup (before any widgets build):
//! register_component("Button", |props: &ComponentProps| {
//!     raxon_view::boxed(text(props.label.clone()).padding(8.0))
//! });
//! ```
...
///     raxon_view::boxed(raxon_view::text(props.label.clone()))
/// });
/// ```
...
/// # Example
/// ```no_run
/// use raxon_view::{resolve_component, ComponentProps, button};
///
/// fn my_button(label: &str, on_press: impl Fn() + 'static) -> impl raxon_view::View {
///     let props = ComponentProps::new().label(label);
///     if let Some(custom) = resolve_component("Button", &props) {
///         return custom;
///     }
///     raxon_view::boxed(button(label, on_press))
/// }
/// ```
