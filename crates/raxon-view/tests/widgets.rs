...
use raxon_dom::{Attribute, Event, Host, Mutation, RecordingBackend, TextSelection, Tree};
use raxon_view::{image, segmented, slider, stepper, switch, text_input, View};
...
fn text_styling_emits_weight_italic_align() {
    ...
    assert!(has(Attribute::TextAlign(raxon_dom::TextAlign::Center)));
}

#[test]
fn indicators_create_and_progress_sets_value() {
    use raxon_dom::WidgetKind;
    use raxon_view::{activity_indicator, progress};
    let (mut tree, log) = harness();
