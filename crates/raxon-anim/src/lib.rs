...
//!
//! ```
//! use raxon_anim::{animate, tick, Easing};
...
pub fn animate(from: f32, to: f32, duration: f32, easing: Easing) -> Signal<f32> {
    let signal = raxon_reactive::create_signal(from);
    ACTIVE.with(|a| {
...
///
/// ```
/// use raxon_anim::{spring, tick, Spring};
/// use raxon_reactive::create_root;
///
...
/// # Example
/// ```rust
/// use raxon_anim::{animate, parallel, Easing};
/// use raxon_reactive::create_root;
///
...
/// frame-perfect sequencing, use a timer future via `raxon_async::spawn_local`.
///
/// # Example
/// ```rust
/// use raxon_anim::{animate, sequence, tick, Easing};
/// use raxon_reactive::{create_root, create_signal};
///
...
pub fn sequence(first: Signal<f32>, to: f32, second: impl FnOnce() + 'static) {
    ...
    raxon_reactive::create_effect(move || {
        if fired.get() {
            return;
        }
...
/// # Example
/// ```rust
/// use raxon_anim::{stagger, animate, Easing};
/// use raxon_reactive::create_root;
///
...
/// # Example
/// ```rust
/// use raxon_anim::{oscillate, tick, Easing};
/// use raxon_reactive::create_root;
///
...
pub fn oscillate(from: f32, to: f32, duration: f32, easing: Easing) -> Signal<f32> {
    let v = raxon_reactive::create_signal(from);
    let dir = raxon_reactive::create_signal(true); // true = forward
...
/// `raxon_async::spawn_local` with a simple `Signal<bool>` trigger instead.
///
/// # Example
/// ```rust
/// use raxon_anim::{delayed, tick, Easing};
/// use raxon_reactive::{create_root, create_signal};
///
...
pub fn delayed(
    ...
    let value = raxon_reactive::create_signal(from);
    raxon_reactive::create_effect(move || {
        if start_trigger.get() {
            let anim = animate(from, to, duration, easing);
            raxon_reactive::create_effect(move || value.set(anim.get()));
        }
    });
    value
}
...
/// # Example
/// ```
/// use raxon_anim::{OffThreadValue, animate_offthread, Easing};
///
...
/// # Example
/// ```no_run
/// use raxon_anim::start_animation_thread;
///
...
thread_local! {
    static REDUCED_MOTION: std::cell::Cell<Option<raxon_reactive::Signal<bool>>> =
...
/// # Example
/// ```rust
/// use raxon_anim::use_reduced_motion;
/// use raxon_reactive::create_root;
///
...
/// scope.dispose();
/// ```
pub fn use_reduced_motion() -> raxon_reactive::Signal<bool> {
    if let Some(s) = REDUCED_MOTION.with(|c| c.get()) {
        return s;
    }
    let s = raxon_reactive::create_signal(false);
...
/// ```rust
/// use raxon_anim::set_reduced_motion;
///
...
///   [`animate`] on a previous frame, or with `raxon_reactive::create_signal`).
...
/// # Example
/// ```rust
/// use raxon_anim::{animate_unless_reduced, use_reduced_motion, Easing};
/// use raxon_reactive::{create_root, create_signal};
///
...
pub fn animate_unless_reduced(
    signal: raxon_reactive::Signal<f32>,
    target: f32,
    duration: f32,
    easing: Easing,
) {
    if use_reduced_motion().get() {
        ...
        raxon_reactive::create_effect(move || signal.set(anim.get()));
    }
}
...
{
    let opacity = raxon_reactive::create_signal(1.0_f32);
    let last_key: Signal<Option<K>> = raxon_reactive::create_signal(None);
...
pub fn keyframes(clock: raxon_reactive::Signal<f32>, frames: Vec<Keyframe<f32>>) -> raxon_reactive::Signal<f32> {
    assert!(!frames.is_empty(), "keyframes requires at least one frame");
    let out = raxon_reactive::create_signal(frames[0].value);
    raxon_reactive::create_effect(move || {
...
pub fn use_looping_clock(period_frames: u64) -> raxon_reactive::Signal<f32> {
    let sig = raxon_reactive::create_signal(0.0f32);
    raxon_reactive::create_effect(move || {
