...
//!
//! The engine stores a neutral [`LayoutStyle`](raxon_core::LayoutStyle) on each
...
use raxon_core::{FlexDirection, LayoutStyle, Rect, Size};
use raxon_dom::{Tree, WidgetId, WidgetKind};
use taffy::prelude::*;
...
fn to_taffy_style(style: LayoutStyle) -> Style {
    Style {
        ...
        position: match style.position {
            raxon_core::Position::Relative => taffy::Position::Relative,
            raxon_core::Position::Absolute => taffy::Position::Absolute,
        },
        ...
        flex_wrap: match style.wrap {
            raxon_core::FlexWrap::NoWrap => taffy::FlexWrap::NoWrap,
            raxon_core::FlexWrap::Wrap => taffy::FlexWrap::Wrap,
            raxon_core::FlexWrap::WrapReverse => taffy::FlexWrap::WrapReverse,
        },
...
fn to_align(a: raxon_core::AlignItems) -> taffy::AlignItems {
    use raxon_core::AlignItems as A;
    match a {
...
fn to_justify(j: raxon_core::JustifyContent) -> taffy::JustifyContent {
    use raxon_core::JustifyContent as J;
    match j {
...
fn to_lp_rect(e: raxon_core::EdgeInsets) -> taffy::Rect<LengthPercentage> {
    taffy::Rect {
        left: length(e.left),
...
fn to_dim(d: raxon_core::Dimension) -> Dimension {
    match d {
        raxon_core::Dimension::Auto => auto(),
        raxon_core::Dimension::Points(p) => length(p),
        raxon_core::Dimension::Percent(p) => percent(p / 100.0),
    }
}
