...
pub fn animate(from: f32, to: f32, duration: f32, easing: Easing) -> Signal<f32> {
    let signal = crate::reactive::create_signal(from);
    ACTIVE.with(|a| {
...
/// frame-perfect sequencing, use a timer future via `crate::async_rt::spawn_local`.
///
/// # Example
/// ```rust
...
pub fn oscillate(from: f32, to: f32, duration: f32, easing: Easing) -> Signal<f32> {
    let v = crate::reactive::create_signal(from);
    let dir = crate::reactive::create_signal(true); // true = forward
...
/// `crate::async_rt::spawn_local` with a simple `Signal<bool>` trigger instead.
///
/// # Example
/// ```rust
...
pub fn delayed(
    ...
    let value = crate::reactive::create_signal(from);
    crate::reactive::create_effect(move || {
        if start_trigger.get() {
            let anim = animate(from, to, duration, easing);
            crate::reactive::create_effect(move || value.set(anim.get()));
        }
    });
    value
}
...
thread_local! {
    static REDUCED_MOTION: std::cell::Cell<Option<crate::reactive::Signal<bool>>> =
...
/// scope.dispose();
/// ```
pub fn use_reduced_motion() -> crate::reactive::Signal<bool> {
    if let Some(s) = REDUCED_MOTION.with(|c| c.get()) {
        return s;
    }
    let s = crate::reactive::create_signal(false);
...
///   [`animate`] on a previous frame, or with `crate::reactive::create_signal`).
...
pub fn animate_unless_reduced(
    signal: crate::reactive::Signal<f32>,
    target: f32,
    duration: f32,
    easing: Easing,
) {
    if use_reduced_motion().get() {
        ...
        crate::reactive::create_effect(move || signal.set(anim.get()));
    }
}
...
{
    let opacity = crate::reactive::create_signal(1.0_f32);
    let last_key: Signal<Option<K>> = crate::reactive::create_signal(None);
...
pub fn keyframes(clock: crate::reactive::Signal<f32>, frames: Vec<Keyframe<f32>>) -> crate::reactive::Signal<f32> {
    assert!(!frames.is_empty(), "keyframes requires at least one frame");
    let out = crate::reactive::create_signal(frames[0].value);
    crate::reactive::create_effect(move || {
...
pub fn use_looping_clock(period_frames: u64) -> crate::reactive::Signal<f32> {
    let sig = crate::reactive::create_signal(0.0f32);
    crate::reactive::create_effect(move || {
