...
- ✅ structured logging (`raxon-log`: `rax_debug!/info!/warn!/error!` macros; `Level` enum; `set_min_level`; routes to `println!` on iOS → Xcode console)
...
- ✅ `raxon-testing` crate: `assert_signal_eq`, `assert_signal`, `assert_after_updates`, `Recorder<T>` (signal change recorder), `with_test_scope`
