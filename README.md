...
| Android backend | 🟡 command backend + driver + binding runtime + generated host glue + Activity shell + host session/registry/versioned bridge + command/event wire |
| Web/WASM backend | 🟡 DOM command backend + driver + binding runtime + generated host glue + browser shell + host session/registry/versioned bridge + command/event wire |
...
raxon generate --target all                      # Android/Web host glue + shells
```
